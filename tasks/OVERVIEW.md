...
- **已完成**：项目资料分析；任务目录创建；TASK-00（Compose 底座、DataStore 设置/主题/语言持久化、核心数据模型、
  ...
  防止重建重复导入），
  TASK-07 Stage AD（待转换文件列表信息密度优化：顶部改为 56dp 紧凑选择栏，列表外边距/分组卡片间距/卡片内
...
- **未解决问题 / 风险**：
  ...
  - **新换上的 FFmpeg 依赖同样静态链接了 x264（GPL）**：排查替代依赖时发现，包括原依赖和新依赖在内的
    ...
    GPL-3.0 完整许可证文本链接，履行 GPL §6(d) 网络分发场景下的对应源代码访问义务。**2026-06-24**：
    进一步将仓库整体设置为 GPL-3.0（新增 `LICENSE`，`README.md` 同步更新），消除此前"本应用自身代码
    未随之开源"的残留风险，详见 TASK-05「残留风险解决」。
