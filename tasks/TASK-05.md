...
### 残留风险解决：仓库整体采用 GPL-3.0（2026-06-24）

按上一节列出的两个解法之二——"将本应用整体开源"，用户决策为仓库设置 GPL-3.0 协议，不再依赖
"FFmpeg 作为独立 GPL 组件随附"的行业惯例论证，从根本上消除单一合成作品判定的不确定性。

- 仓库根目录新增 `LICENSE`（FSF 官方 GPL-3.0 全文，原文未改动）。
- `README.md` 「许可证与开源组件」章节更新为"本项目以 GPL-3.0 开源"，并补充 License 徽章。
- 应用内「设置 → 关于 → 开源组件」文案待后续与本仓库协议保持一致性核对（未在本次改动范围内）。
